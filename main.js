...
VEL_CONST = 5;
DIVERGENCE_ITERATIONS_COUNT = 500;
DISPLAY_PARAM = 0; //0 for pressure, 1 for velocity
...
const traverse_cells = () => {
  ...
  for (
    ...
    main_iteration < DIVERGENCE_ITERATIONS_COUNT;
    main_iteration++
  ) {
    for (let j = SIM_HEIGHT - 2; j > 0; j--) {
...
const create_obstacle = () => {
  for (
    ...
    for (
      ...
      let cell_current =
        cell_array[convertTo1D(x_offset + y_offset - SIM_HEIGHT / 2, y_offset)];
