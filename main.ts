...
const normaliseValueToColour2 = (value: number, normalizationFactor: number): number => {
  ...
  drawFluid = (ctx: CanvasRenderingContext2D, scalingFactor: number): void => {
    ...
    for (let i = 0; i < this.cellCount.x; i++) {
      for (let j = 0; j < this.cellCount.y; j++) {
        ...
        // this.resetPressure();
        ctx.fillStyle = `rgb(${Math.abs(valueToUseToColour) * 255}, 0, ${
          (1 - Math.abs(valueToUseToColour)) * 255
        })`;
        if (cell.isWall) {
          ctx.fillStyle = "black";
        }
...
const addWallFromClick = (e: MouseEvent) => {
  const rect = canvas.getBoundingClientRect();
  const x = e.clientX - rect.left;
  const y = e.clientY - rect.top;
